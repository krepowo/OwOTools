...
export default {
    ...
    run: async (interaction) => {
        ...
        try {
            const data = await fetchRyzumiAPI('/search/wallpaper-moe', { query: query });

            const wallpaper = data.result;

            if (wallpaper.length === 0) {
                const embed = createSimpleEmbed(`No valid wallpaper results found for: **${query}**`, 'Wallpaper Search', '#FF0000');
            ...
            const generateEmbed = (index) => {
                ...
                return new EmbedBuilder()
                    ...
                    .setFooter({ text: `Video ${index + 1} of ${wallpaper.length} | Searched for: ${query}` });
            };
