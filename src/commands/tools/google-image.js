...
export default {
    ...
    run: async (interaction) => {
        ...
        try {
            const data = await fetchRyzumiAPI('/search/gimage', { query: query });

            const filteredResults = data
                .filter(image => image.image && image.image.match(/\.(jpg|jpeg|png|gif)$/i) && isDiscordSafe(image.image))
                .map(image => ({
                    title: image.title,
                    image: image.image,
                    url: image.url
            ...
            if (filteredResults.length === 0) {
                const embed = createSimpleEmbed(`No valid image results found for: **${query}**`, 'Google Image Search', '#FF0000');
            ...
            const generateEmbed = (index) => {
                ...
                return new EmbedBuilder()
                    ...
                    .setFooter({ text: `Image ${index + 1} of ${filteredResults.length} | Searched for: ${query}` });
            };
